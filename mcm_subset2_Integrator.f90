...
! 
! File                 : mcm_subset2_Integrator.f90
...
MODULE mcm_subset2_Integrator

  USE mcm_subset2_Precision
  USE mcm_subset2_Global, ONLY: FIX, RCONST, TIME
  USE mcm_subset2_Parameters, ONLY: NVAR, NSPEC, NFIX, LU_NONZERO
  USE mcm_subset2_JacobianSP, ONLY: LU_DIAG
  USE mcm_subset2_LinearAlgebra, ONLY: KppDecomp, KppSolve, &
...
!~~~> Arguments:      
      INTEGER :: N
      REAL(kind=dp), INTENT(INOUT) :: Y(NVAR)
      REAL(kind=dp), INTENT(IN) :: Tinitial, Tfinal, Hmin, Hmax, Hstart, &
                        RelTol(NVAR), AbsTol(NVAR), Roundoff,            &
                        FacMin, FacMax, FacRej, FacSafe, ThetaMin,       &
                        NewtonTol, Qmin, Qmax
      REAL(kind=dp), INTENT(OUT) :: Hexit, Texit
      INTEGER, INTENT(IN)  :: ITOL, Max_no_steps, NewtonMaxit
      INTEGER, INTENT(OUT) :: IDID
      
!~~~> Local variables:      
      REAL(kind=dp) :: Z(NVAR,5), FV(NVAR,5), CONT(NVAR,4),      &
      ...
      INTEGER :: i, j, ISING, istage, NewtonIter, IP(NVAR)
...
!~~~>  Compute E = 1/(h*gamma)-Jac and its LU decomposition
      ...
      ELSE
         CALL SDIRK_PrepareMatrix ( H, T, Y, FJAC, &
                   FreshJac, SkipJacUpdate, E, IP, Reject, ISING )
         IF (ISING /= 0) THEN
...
#ifdef FULL_ALGEBRA  
      CALL DGETRS( 'N', N, 1, E, N, IP, TMP, N, ISING )
#else
      CALL KppSolve(E, TMP)
#endif
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      USE mcm_subset2_Parameters, ONLY: NVAR, LU_NONZERO
      USE mcm_subset2_Global, ONLY: TIME, FIX, RCONST
      USE mcm_subset2_Function
      USE mcm_subset2_Rates, ONLY: Update_SUN, Update_RCONST
...
END MODULE mcm_subset2_Integrator
